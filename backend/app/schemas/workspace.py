...
class AssetPresignResponse(BaseModel):
    ...
    hint: str = "Prefer POST /v1/workspace/assets/upload with multipart form."
...
class AssetOut(BaseModel):
    ...
    @classmethod
    def from_row(cls, row: Mapping[str, object]) -> "AssetOut":
        sh = row.get("sha256")
        hex_digest: str | None = None
        if isinstance(sh, (bytes, memoryview)):
            hex_digest = bytes(sh).hex()
        return cls(
            id=row["id"],  # type: ignore[assignment]
...
class AssetsListPayload(BaseModel):
    items: list[AssetOut]
