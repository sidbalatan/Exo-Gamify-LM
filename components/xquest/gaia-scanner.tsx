...
function mockGaiaSourceId(): string {
  // Gaia DR3 source IDs are ~18-19 digit integers, typically 4xxxGÇô6xxx +ù 10^15
    ...
    try {
      const res = await fetch("/api/workspace/targets", {
        ...
        body: JSON.stringify({
          ...
          label: `Level ${result.candidateLevel} GÇö ${result.constellation}`,
        }),
      })
  ...
  return (
    <div className="space-y-2">
      <Button size="sm" className="w-full" onClick={saveAll} disabled={saving.size > 0}>
        {saving.size > 0 ? (
          <><Loader2 className="mr-2 h-4 w-4 animate-spin" />SavingGÇª</>
        ) : (
...
export function GaiaScanner() {
  ...
  const handleStarClick = () => {
    ...
    setStarTooltip(`Temp: ${randomTemp}K GÇó Mass: ${randomMass} MGÿë`)
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {result.constellation} GÇó Mag {result.magnitude.toFixed(2)}
                      </p>
                    </div>
