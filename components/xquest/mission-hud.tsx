...
const facts = [
  ...
  "Climate change could make Earth uninhabitable in the next 1,000 years GÇö finding alternatives is critical.",
  ...
  "Exoplanets in the habitable zone may contain liquid water GÇö the foundation of life as we know it.",
]

// Rendered only inside ClerkProvider GÇö safe to call useAuth()
  ...
  useEffect(() => {
    ...
    queueMicrotask(async () => {
      try {
        ...
        // non-fatal GÇö stat simply won't show
      }
    })
      ...
      <Link href="/sign-in">
        <div className="relative overflow-hidden rounded-lg border border-primary/30 bg-primary/5 p-3 sm:p-4 backdrop-blur-sm transition-colors hover:bg-primary/10">
          ...
          <div className="relative">
            ...
            <p className="font-mono text-lg font-bold sm:text-2xl text-primary">GÇö</p>
            <p className="text-[10px] sm:text-xs leading-tight text-muted-foreground">
              My Targets
            </p>
            <p className="mt-1 text-[9px] text-primary/70">Sign in GåÆ</p>
          </div>
        </div>
      </Link>
    )
      ...
      <div className="relative overflow-hidden rounded-lg border border-primary/30 bg-primary/5 p-3 sm:p-4 backdrop-blur-sm transition-colors hover:bg-primary/10">
        ...
        <div className="relative">
          ...
          <p className="font-mono text-lg font-bold sm:text-2xl text-primary">
            {count === null ? "GÇª" : count}
          </p>
          <p className="text-[10px] sm:text-xs leading-tight text-muted-foreground">
            My Targets
          </p>
          <p className="mt-1 text-[9px] text-primary/70">Open workspace GåÆ</p>
        </div>
      </div>
    </Link>
  )
